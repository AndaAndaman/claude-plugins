...
# Module: claude-plugins Repository

## Overview

A Claude Code plugin marketplace for FlowAccount developers containing production-ready productivity plugins. Repository hosts the flowaccount-dev-tools marketplace with three complementary plugins: ask-before-code (requirement clarity), quick-wins (code quality maintenance), and md-to-skill (markdown to skill conversion).

## Files

### README.md
Main documentation hub for the repository. Contains quick start guide, plugin comparison table, installation instructions (marketplace, local, global, temporary testing), FlowAccount-specific compatibility details, and roadmap. Lists plugin status, metrics table showing ROI (6+ hours saved per feature), and contribution guidelines.

### CLAUDE.md
Project instructions for Claude Code sessions. Documents official reference links (Plugins, Hooks documentation), repository structure, plugin architecture details, development workflow, testing procedures, hook response schemas, version management, marketplace configuration, and FlowAccount context. Includes patterns for agent descriptions, task invocation, and hook responses.

### .claude-plugin/marketplace.json
Marketplace configuration for flowaccount-dev-tools. Contains marketplace name, owner (andaman_n@flowaccount.com), repository URL, and plugin catalog entries with versions and sources.

### BUG_REPORT.md
Documented bug report: SessionStart hooks incorrectly deduplicated across plugins. Describes the issue, reproduction steps, expected vs actual behavior, and impact on multi-plugin scenarios.
...
**Plugin Structure** — Each plugin follows consistent layout: .claude-plugin/plugin.json (metadata), agents/ (autonomous subprocesses), commands/ (user-invocable slash commands), hooks/ (event-driven automation), skills/ (reusable knowledge bases with progressive disclosure).

**Hook Response Schema** — Prompt-based hooks return `{"ok": boolean, "reason"?: string}`. Command-based hooks return exit code 0 or `{"decision": "block", "reason": string}`. Official https://code.claude.com/docs/en/hooks.md is authoritative source.

**Hook Naming Convention** — Hook scripts follow pattern `{plugin-name}_{hook-type}.py` (e.g., `quick-wins_stop.py`, `local-memory_stop.py`) to immediately identify parent plugin.

**FlowAccount Compatibility** — Plugins respect FlowAccount naming conventions (`_underscore` prefix), Clean Architecture layers, Nx boundaries, DDD structure, and component prefixes. Fully compatible with both flowaccount.workspace (TypeScript/Angular) and flowaccount.dotnet.workspace (.NET/C#).

**Version Synchronization** — Plugin versions stored in both individual plugin.json files and .claude-plugin/marketplace.json; must be kept synchronized when bumping versions.

## Dependencies

**Produces:**
- Plugin marketplace for installation via `/plugin marketplace add AndaAndaman/claude-plugins`
- Individual plugin installations (ask-before-code, quick-wins, md-to-skill)

**External References:**
- Claude Code official documentation (https://code.claude.com/docs/en/)
- GitHub repository (https://github.com/AndaAndaman/claude-plugins)

**Internal Components:**
- plugins/ask-before-code — Requirement clarification plugin
- plugins/quick-wins — Code quality scanning plugin
- plugins/md-to-skill — Markdown to skill conversion plugin (v0.8.2+)
