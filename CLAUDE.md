...
<!-- AUTO-GENERATED by local-memory plugin on 2026-02-24 -->
<!-- Updates: 2026-02-25, 2026-02-24 -->
<!-- To preserve custom content, add sections outside auto-gen blocks -->

# Module: claude-plugins Repository Root

## Overview

A Claude Code plugin marketplace for FlowAccount developers hosting five production plugins that enhance development workflows. Repository serves as the official flowaccount-dev-tools marketplace with comprehensive documentation, testing infrastructure, and multi-plugin integration examples.

## Files

### README.md
Primary documentation hub with quick start guide, plugin overview table, installation instructions (marketplace, local, global, temporary testing), and example usage. Documents all five plugins with command references, version numbers, and FlowAccount compatibility details. Includes contribution guidelines and development workflow notes.

### CLAUDE.md
Master project instructions for Claude Code sessions. Contains official documentation references (Plugins Reference, Hooks Reference), complete repository structure diagram, detailed plugin architecture sections for all plugins, development workflow guidelines, testing procedures, hook response schemas with examples, version management strategy, marketplace configuration, FlowAccount integration context, and key concepts. Serves as authoritative guide for plugin development and modification.

### BUG_REPORT.md
Documented issue: SessionStart hooks incorrectly deduplicated across plugins when multiple plugins define hooks for the same event. Includes reproduction steps, expected vs actual behavior, and impact analysis on plugin initialization.

### LICENSE
MIT License file.

### .claude-plugin/marketplace.json
Marketplace configuration for flowaccount-dev-tools. Contains marketplace name, owner email (andaman_n@flowaccount.com), GitHub repository URL, and catalog entries for all five plugins with version numbers and source paths.

## Key Patterns

**Five-Plugin Architecture** — Repository hosts: ask-before-code (requirement clarity), quick-wins (code quality), md-to-skill (learning engine), local-memory (context building), and feature-sprint (sprint planning). All plugins follow consistent structure: .claude-plugin/plugin.json, agents/, commands/, hooks/, skills/.

**Master CLAUDE.md Strategy** — Root CLAUDE.md is tracked in git (primary project instructions), all subdirectory CLAUDE.md files ignored. This enables rich project guidance while keeping local context ephemeral. Updated via local-memory plugin when plugins or documentation changes.

**Official Documentation Requirement** — Plugin modifications must consult https://code.claude.com/docs/en/plugins-reference.md and https://code.claude.com/docs/en/hooks.md. CLAUDE.md provides repository context but defers to official docs for schemas and specifications.

**Marketplace Management** — Versions tracked in both individual plugin.json files and .claude-plugin/marketplace.json; must be synchronized. Marketplace installable via `/plugin marketplace add AndaAndaman/claude-plugins`.

**Hook Script Naming** — Command-based hooks use pattern `{plugin-name}_{hook-type}.py` (e.g., `quick-wins_stop.py`, `md-to-skill_observe.py`) for immediate plugin identification in filesystem.

**FlowAccount Compatibility** — All plugins respect FlowAccount naming conventions, Clean Architecture layers, Nx boundaries, DDD structure. Compatible with both flowaccount.workspace (TypeScript/Angular) and flowaccount.dotnet.workspace (.NET/C#).

## Dependencies

**Produces:**
- flowaccount-dev-tools marketplace for plugin installation
- Five installable plugins with independent versioning
- CLAUDE.md context files (root tracked, subdirectories ignored)

**External Dependencies:**
- Claude Code platform (plugin host)
- GitHub (repository storage and marketplace source)
- Official Claude Code documentation (https://code.claude.com/docs/en/)

**Internal Component Hierarchy:**
- plugins/ask-before-code — Requirement clarity (v0.4.0)
- plugins/quick-wins — Code quality scanning (v0.3.4)
- plugins/md-to-skill — Learning engine (v0.8.4)
- plugins/local-memory — Context generation (provides this CLAUDE.md)
- plugins/feature-sprint — Sprint planning



## Timeline

- **2026-02-25** — Updated root context to document five plugins (added local-memory and feature-sprint), marketplace strategy, hook naming conventions, and FlowAccount compatibility
- **2026-02-24** — Documented marketplace structure, plugin architecture, hook patterns, FlowAccount compatibility, and version management for three production plugins
