...
| **Jenkins** | `jenkins_configure`, `jenkins_list_targets`, `jenkins_build`, `jenkins_status`, `jenkins_abort`, `jenkins_edit_config` | Full CI lifecycle: configure, trigger, monitor, abort. Build auto-resolves queue→build number. Status checks last build if no URL. |
| **Healthcheck** | `healthcheck` | Check endpoints, manage the endpoint list |

**Agent:** `bg-runner` — runs long-running MCP operations (build monitoring, ECS wait, post-build verification) in background so you can keep working.
...
| dev-tools | 0.9.15 | Native MCP tools for git, HTTP, AWS, Jenkins, health checks |
