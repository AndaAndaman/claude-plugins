...
var CONFIG_DIR = (0, import_node_path.join)((0, import_node_os.homedir)(), ".config", "dev-tools");
...
function load() {
  if (current)
    return current;
  try {
    const raw = (0, import_node_fs.readFileSync)(CONFIG_FILE, "utf8");
...
function runAws(args) {
  const result = (0, import_node_child_process.spawnSync)("aws", args, { encoding: "utf8", maxBuffer: 10 * 1024 * 1024 });
  return {
...
function runAwsWithProfile(args) {
...
function defineTool(server2, name, description, inputSchema, handler) {
  server2.registerTool(name, { description, inputSchema }, handler);
}

// src/tools/ecs-list.tool.ts
function registerEcsListTool(server2) {
  defineTool(
    server2,
    "aws_ecs_list",
    "Lists ECS services filtered by tag key/value, grouped by cluster with service status.",
    {
      tagValue: external_exports.string().optional().describe(`Tag value to filter by (default: "${getTagValue()}")`)
    },
    async (input) => {
      try {
        ...
        const lines = [`Fetching ECS services with tag ${getTagKey()}=${tagValue}...
`];
        for (const [cluster, services] of clusterMap) {
          ...
          const descResult = runAwsWithProfile([
            ...
            ...services,
            "--query",
            "services[].{Service:serviceName,Desired:desiredCount,Running:runningCount,Status:status}",
            "--output",
            "table"
          ]);
...
// src/tools/ecs-scale.tool.ts
function registerEcsScaleTool(server2) {
  defineTool(
    server2,
    "aws_ecs_scale",
    ...
    {
      tagValue: external_exports.string().optional().describe(`Tag value to filter by (default: "${getTagValue()}")`),
      confirm: external_exports.boolean().describe("Set true to actually perform the scaling (default: false = preview only)")
    },
    async (input) => {
      const tagValue = input.tagValue ?? getTagValue();
      const confirm = input.confirm ?? false;
      const lines = [`Finding services with tag ${getTagKey()}=${tagValue}...`];
      try {
        const arnResult = runAwsWithProfile([
          "resourcegroupstaggingapi",
          "get-resources",
...
Scaling up service ${service} in cluster ${cluster} to desired count 1...`);
          const updateResult = runAwsWithProfile([
            ...
            "table",
            "--query",
            "service.[serviceName,desiredCount,runningCount]"
          ]);
...
// src/tools/ecs-update-service.tool.ts
function registerEcsUpdateServiceTool(server2) {
  defineTool(
    server2,
    "aws_ecs_update_service",
    "Update a single ECS service desired count. Use confirm=false to preview, confirm=true to execute.",
    {
      cluster: external_exports.string().describe("ECS cluster name"),
      service: external_exports.string().describe("ECS service name"),
      desiredCount: external_exports.number().describe("New desired count for the service (integer >= 0)"),
      confirm: external_exports.boolean().describe("Set true to actually perform the update (default: false = preview only)")
    },
    async (input) => {
      const cluster = input.cluster;
      const service = input.service;
      const desiredCount = input.desiredCount;
      const confirm = input.confirm ?? false;
      try {
        if (!confirm) {
          return textResult(
...
Set confirm=true to perform the update.`
        ...
        const lines = [`Updating service ${service} in cluster ${cluster} to desired count ${desiredCount}`];
...
// src/tools/set-profile.tool.ts
function registerConfigureTool(server2) {
  defineTool(
    server2,
    "aws_configure",
    ...
    {
      profile: external_exports.string().optional().describe("AWS CLI profile name (default: basic_profile)"),
      tagKey: external_exports.string().optional().describe("ECS service tag key for filtering (default: acc-sandbox)"),
      tagValue: external_exports.string().optional().describe("ECS service tag value for filtering (default: core,profile,report,doc,ui,my)")
    },
    async (input) => {
...
var SSO_PROFILE = "sso";
var SSO_CRED_PROFILE = "265515193476_DeveloperAccessRole@697698820969";
function getSsoExpiry() {
  const cacheDir = (0, import_node_path2.join)((0, import_node_os2.homedir)(), ".aws", "sso", "cache");
  let files;
  try {
    files = (0, import_node_fs2.readdirSync)(cacheDir).filter((f) => f.endsWith(".json"));
  } catch {
    return null;
  }
  const withMtime = files.map((f) => {
    const full = (0, import_node_path2.join)(cacheDir, f);
    try {
      const raw = (0, import_node_fs2.readFileSync)(full, "utf8");
      const data = JSON.parse(raw);
      if (data.providerType)
        return null;
      if (!data.expiresAt)
        return null;
      return { path: full, expiresAt: data.expiresAt };
    } catch {
      return null;
    }
  }).filter(Boolean);
  ...
  if (isNaN(expiresEpochMs))
    return null;
  return { expiresAt: best.expiresAt, expiresEpochMs };
}

// src/tools/sso-status.tool.ts
function registerSsoStatusTool(server2) {
  defineTool(
    server2,
    "aws_sso_status",
...
// src/tools/sso-refresh.tool.ts
function registerSsoRefreshTool(server2) {
  defineTool(
    server2,
    "aws_sso_refresh",
    ...
    {
      force: external_exports.boolean().optional().describe("Force re-login even if session is still valid (default: false)")
    },
    async (input) => {
      ...
      const expiry = getSsoExpiry();
      const nowMs = Date.now();
      const isValid2 = expiry && expiry.expiresEpochMs - nowMs > 0;
      if (isValid2 && !force) {
...
function registerTools(server2) {
  ...
  registerEcsListTool(server2);
  registerEcsScaleTool(server2);
